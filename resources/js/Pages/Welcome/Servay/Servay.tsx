  ...
  const renderProgressBar = () => (
    <div className="container flex justify-between items-center gap-4 relative after:absolute after:h-[1px] after:w-full after:bg-black after:-z-10">
      {[1, 2, 3, 4, 5].map((step) => (
        <div
          key={step}
          className={`w-10 h-10 text-center flex justify-center items-center rounded-full border-2 border-white ${
            currentStep >= step ? 'bg-white text-black' : 'bg-primary-color text-white'
          }`}
        >
          {step}
        </div>
