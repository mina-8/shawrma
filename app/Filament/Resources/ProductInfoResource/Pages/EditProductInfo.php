...
class EditProductInfo extends EditRecord
{
    ...
    protected function getDefaultCoreStations(): array
    {
        return CoreStation::where('stationable_id', $this->record->id)
            ->where('stationable_type', ProductInfo::class) // Assuming you're using a 'type' column to distinguish them
            ->get()
    ...
    protected function afterSave(): void
    {
        ...
        $ExistCoreStation = CoreStation::where('stationable_id', $this->record->id)
            ->where('stationable_type', ProductInfo::class)
            ->pluck('id')->toArray();
